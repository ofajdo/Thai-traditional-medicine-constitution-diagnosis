...
let img_x_position = -1464; // 初期x座標
let img_y_position = 0; // 初期x座標
const img_x_size = 4392; // 画像サイズ
const img_y_size = 200; // 画像サイズ
...
const DrawImage = () => {
  ...
  ctx.drawImage(img, img_x_position, img_y_position, img_x_size, img_y_size);
};
...
const Medical = (value) => {
  ...
  if (date) {
    const days = FromDate("2024-" + date.date + default_time);
    return days;
  }
};
